...
async def Canesel_Key(c: Client, m: Message, userid):
    ...
    await m.reply(
        ...
        reply_markup=ReplyKeyboardMarkup(
            await orm.GetMainKeys(userid), resize_keyboard=True
        ),
    )
...
async def CheckBlockUser(_, c: Client, m: Message):
    ...
    if await orm.CheckUserBlock(m.from_user.id) == True:
        ...
        await c.send_message(
            m.from_user.id,
            text=Messages["BlockUser"],
            reply_markup=ReplyKeyboardMarkup([["✅ درخواست"]], resize_keyboard=True),
        )
        return False
    else:
        return True
    ...
    try:
        ...
        await c.send_message(
            ...
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("ورود", url=f"https://t.me/{chanel}")]]
            ),
        )
        return False
...
async def CheckBtnsNot(_, c: Client, m: Message):
    ...
    # بررسی تطابق با حذف کاراکترهای ترکیبی
...
async def REQOnBlock(c: Client, m: Message):
    if await orm.CheckUserBlock(m.from_user.id) == True:
        ...
        await m.reply(
            "کاربر گرامی شما رفع مسدودیت شدید یا مسدود نیستید ! 🟢",
            reply_markup=ReplyKeyboardMarkup(
                await orm.GetMainKeys(m.from_user.id), resize_keyboard=True
            ),
        )
...
""",
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "پاسخ ✍🏻", callback_data=f"answerTiket_{res[1]}"
                            )
                        ]
                    ]
                ),
                        ...
                        reply_markup=InlineKeyboardMarkup(
                            [
                                [
                                    InlineKeyboardButton(
                                        "پاسخ ✍🏻",
                                        callback_data=f"answerTiket_{res[1]}",
                                    )
                                ]
                            ]
                        ),
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "رفع مسدودیت 🟢",
                            callback_data=f"Requnblock_{m.from_user.id}",
                        ),
                        InlineKeyboardButton(
                            "رد", callback_data=f"radBlock_{m.from_user.id}"
                        ),
                    ]
                ]
            ),
...
async def EnterDiscount(c: Client, m: Message, orderId):
    ...
    if dis[0] == True:
        ...
        await orm.ChangeStep(m.from_user.id, "home")
        await m.reply(
            f"""🎯 | در این بخش شیوه پرداخت خود را انتخاب کنید
    ...
    try:
        ...
        if orderPrice[3] == "BuySingle":
            ...
            await c.send_cached_media(
                chat_id=dataConfig["ownerId"],
                file_id=m.photo.file_id,
                caption=f"""🛍 خرید سرویس 
...
🔰 /start                                 
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "✅ تایید", callback_data=f"SuccessCard_{data}"
                            ),
                            InlineKeyboardButton(
                                "❌ رد کردن ", callback_data=f"aboveCard_{data}"
                            ),
                        ]
                    ]
                ),
...
🔰 /start                                 
        ...
        await m.reply(
            text=f"""🔰 | لطفا ابتدا به این شماره کارت مبلغ را واریز کنید 
...
🔰 /start                                 
        ...
        await m.reply_video(
            "https://media.giphy.com/media/edckP6sD9YyYxuQYpS/giphy.gif",
            caption="/:",
            reply_markup=ReplyKeyboardMarkup(
                await orm.GetMainKeys(m.from_user.id), resize_keyboard=True
            ),
...
async def StartBot(c: Client, m: Message):
    ...
    await orm.AddNewUser(
        m.from_user.id, m.from_user.first_name, m.from_user.username, command, c, m
    )
        ...
        await m.reply_photo(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "🟢" if res[2]["state"] == 1 else "🔴", callback_data="ARS"
                        ),
                        InlineKeyboardButton("🔔 وضعیت", callback_data="ARS"),
                    ],
                    [
                        InlineKeyboardButton(total, callback_data="ARS"),
                        InlineKeyboardButton("🔋 حجم کل", callback_data="ARS"),
                    ],
                    [
                        InlineKeyboardButton(round(used, 2), callback_data="ARS"),
                        InlineKeyboardButton("🪫 حجم مصرفی  ", callback_data="ARS"),
                    ],
                    [
                        InlineKeyboardButton(round(mande, 2), callback_data="ARS"),
                        InlineKeyboardButton(
                            "🪫 حجم باقی مانده  ", callback_data="ARS"
                        ),
                    ],
                    [
                        InlineKeyboardButton(download, callback_data="ARS"),
                        InlineKeyboardButton("📥 حجم دانلود ", callback_data="ARS"),
                    ],
                    [
                        InlineKeyboardButton(upload, callback_data="ARS"),
                        InlineKeyboardButton("📤 حجم آپلود ", callback_data="ARS"),
                    ],
                    [
                        InlineKeyboardButton(str(shamsi), callback_data="ARS"),
                        InlineKeyboardButton("📆 انقضا", callback_data="ARS"),
                    ],
                    [
                        InlineKeyboardButton(toEnd, callback_data="ARS"),
                        InlineKeyboardButton(
                            "🕐 روز های باقی مانده", callback_data="ARS"
                        ),
                    ],
                ]
            ),
            ...
            if Setting[1] == 1:
                btns.append(
                    InlineKeyboardButton(
                        (
                            "اشتراک معمولی "
                            if Setting[3] == "empty" or Setting[3] == None
                            else f"{Setting[3]}"
                        ),
                        callback_data="SingleShop",
                    )
                )
            if Setting[0] == 1:
                btns.append(
                    InlineKeyboardButton(
                        (
                            "اشتراک ساب "
                            if Setting[2] == "empty" or Setting[2] == None
                            else f"{Setting[2]}"
                        ),
                        callback_data="MultiShop",
                    )
                )
            if Setting[4] == 1:
                btnsonline.append(
                    InlineKeyboardButton(
                        "🌐 | خرید از سایت (تحویل آنی)",
                        url=f"https://xenitgame.com/ping?from=do&userId={call.from_user.id}",
                    )
                )
            if btns != []:
                finalbtns = [btns]
...
🔰 /start              """,
                    reply_markup=InlineKeyboardMarkup(finalbtns),
                )
                return
            else:
                await call.edit_message_text(
    ...
    if "OrderSingle_" in call.data:
        ...
        if user != 0:
            ...
            if Per != None:
                ...
                NewPrice = plan[3]
            order = await orm.CreateOrderSingle(
                call.from_user.id,
                data,
                0,
                NewPrice,
...
🔰 /start              """,
                reply_markup=InlineKeyboardMarkup(await orm.GetCatBtnsShell()),
            )
            return
        else:
            await call.answer(
...
🔰 /start              """,
    ...
    if call.data == "buyPanel":
        await call.edit_message_text(
            "برای خرید پنل همکاری به پشتیبانی پیام بدهید",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("پشتیبانی", url="https://t.me/AR_S_83")]]
            ),
        )

    if call.data == "buyBot":
        await call.edit_message_text(
            "برای خرید ربات به پشتیبانی پیام بدهید",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("پشتیبانی", url="https://t.me/AR_S_83")]]
            ),
        )
    ...
    if "deatilApp_" in call.data:
        ...
        if app[5] == "empty":
            await call.message.reply(
                app[3],
                reply_markup=InlineKeyboardMarkup(
                    [[InlineKeyboardButton(f"دانلود {app[1]}", url=app[2])]]
                ),
            )
        else:
            await c.send_cached_media(
    ...
    if call.data == "mainApp":
        await call.edit_message_text(
            """📲 | از لیست زیر سیستم عامل مورد نظر خود را انتخاب کنید
...
""",
    ...
    if "ReqCooperation_" in call.data:
        ...
        try:
            ...
            await c.send_message(
                data["ownerId"],
                f"""👤 کاربر {call.from_user.username} 
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "✅ تایید",
                                callback_data=f"SuccessPerUse_{perId}_{user[0]}",
                            ),
                            InlineKeyboardButton(
                                "❌ رد", callback_data=f"RejectPerUse_{user[1]}"
                            ),
                        ]
                    ]
                ),
...
🔰 /start""",
    ...
    if call.data == "SendTiket":
        await call.message.reply(
            "⌛️", reply_markup=ReplyKeyboardMarkup([["انصراف"]], resize_keyboard=True)
        )
...
""",
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "👤 زیر مجموعه گیری", callback_data="collection"
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                "🤝 تخفیف همکاری", callback_data="Cooperationdiscount"
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                "🛍 خرید پنل همکاری", callback_data="buyPanel"
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                "🤖 دریافت ربات اختصاصی", callback_data="buyBot"
                            )
                        ],
                    ]
                ),
        ...
        if await orm.CheckWalletForExtension(planId, call.from_user.id) == True:
            ...
            if res[0] == True:
                ...
                await call.message.reply(
                    res[1],
                    reply_markup=ReplyKeyboardMarkup(
                        await orm.GetMainKeys(call.from_user.id), resize_keyboard=True
                    ),
                )
                return
        else:
...
🔰 /start                  
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    ...
                    [
                        InlineKeyboardButton(
                            "بیشترین تعداد خرید", callback_data="upCountShoped"
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            "بیشترین مقدار خرید", callback_data="UpShoped"
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            "بیشترین دعوت کننده", callback_data="UpInviteUsers"
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            "بیشترین مصرف کننده", callback_data="UpUseUsers"
                        )
                    ],
                ]
            ),
...
🔰 /start                                     
    ...
    if "extension_" in call.data:
        if await orm.checkServiceBtn("tamdidbtn") == True:
            ...
            await call.edit_message_text(
                "لطفا پلن خود را انتخاب کنید و برای پرداخت اقدام کنید ",
                reply_markup=InlineKeyboardMarkup(btns),
            )
        else:
...
🔰 /start                                     
    ...
    if "transferWallet" in call.data:
        ...
        await call.message.reply(
            text="🔻بزرگتر از  تومان 10,000 لطفا قیمت را ارسال فرمایید"
        )
        return
    ...
    if "getOrder_" in call.data:
        ...
        if user != 0:
            ...
            if Per != None:
                ...
                NewPrice = plan[3]
            order = await orm.CreateOrder(
                call.from_user.id, data, 0, NewPrice, "BuySub", plan[2], CatId
            )
        else:
            order = await orm.CreateOrder(
                call.from_user.id, data, 0, plan[3], "BuySub", plan[2], CatId
            )
            ...
            await call.message.reply_photo(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        ...
                        [
                            InlineKeyboardButton(
                                str(shamsi.date()), callback_data="ARS"
                            ),
                            InlineKeyboardButton("📆 انقضا", callback_data="ARS"),
                        ],
                        [
                        ...
                        [
                            InlineKeyboardButton(
                                "تعویض لینک سرویس", callback_data=f"Changelink_{data}"
                            ),
                            InlineKeyboardButton(
                                " 🔁 تمدید", callback_data=f"extension_{data}"
                            ),
                        ],
                        [
                            InlineKeyboardButton(
                                "منو سرویس ها", callback_data="menuService"
                            )
                        ],
                    ]
                ),
                ...
                await call.message.reply_photo(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            ...
                            [
                                InlineKeyboardButton(
                                    round(used, 2), callback_data="ARS"
                                ),
                                InlineKeyboardButton(
                                    "🪫 حجم مصرفی   ", callback_data="ARS"
                                ),
                            ...
                            [
                                InlineKeyboardButton(
                                    round(mande, 2), callback_data="ARS"
                                ),
                                InlineKeyboardButton(
                                    "🪫  حجم باقی مانده  ", callback_data="ARS"
                                ),
                            ...
                            [
                                InlineKeyboardButton(download, callback_data="ARS"),
                                InlineKeyboardButton(
                                    "📥 حجم دانلود ", callback_data="ARS"
                                ),
                            ],
                            [
                                InlineKeyboardButton(upload, callback_data="ARS"),
                                InlineKeyboardButton(
                                    "📤 حجم آپلود ", callback_data="ARS"
                                ),
                            ],
                            [
                                InlineKeyboardButton(
...
🔰 /start    """,
            reply_markup=InlineKeyboardMarkup(
                await orm.GetServiceList(call.from_user.id)
            ),
        )
        return
    ...
    if call.data == "SharjWallet":
        ...
        await call.message.reply(
            text="🔻(بیشتر از 10000 تومان) لطفا قیمت را حتما به عدد وارد کنید"
        )
        return
    ...
    if "PayWallet_" in call.data:
        ...
        if res == True:
            ...
            if result[0] == True:
                ...
                await c.send_message(
                    chat_id=result[3],
                    text="""✅ | سرویس شما اضافه با موفقیت خریداری شد 
...
""",
                    reply_markup=InlineKeyboardMarkup(
                        [
                            [
                                InlineKeyboardButton(
                                    "دریافت", callback_data=f"GETConfig_{result[2]}"
                                )
                            ]
                        ]
                    ),
